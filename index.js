$( function() {
    ...
    function mostrarDetalhesMaterial(materialSelecionado) {
      ...
      Número: ${materialSelecionado.numero}, Nome: ${materialSelecionado.nome}, Livre: ${materialSelecionado.livre} 
    ...
    function mostrarMateriaisSemelhantes(materialSelecionado) {
      ...
      materials.forEach(material => {
        if (material.livre === materialSelecionado.livre && material.nome !== materialSelecionado.nome) {
          listaSemelhantes.append(
            `  <li> Número: ${material.numero}, Nome: ${material.nome},
            Quantidade Livre:
             ${material.livre}, 
            <img src="${material.foto}" 
            alt="${material.nome}" width="50"> 
            <button class="reservarMaterialSemelhante" data-numero="${material.numero}"> Reservar</button></li>
            `
            );
        }
      });
    ...
    $(document).on("click", ".reservarMaterialSemelhante", function() {
      ...
      if (numeroReserva !== null) {
        ...
        material.livre = (material.livre -1);
